    ...
    root.render(
        <ThemeProvider theme={kapetaLight}>
            <SplashContent
                ...
                onRestartCluster={() => {
                    window.electron.ipcRenderer.sendMessage('splash', ['restart-cluster']);
                }}

                onCheckDocker={() => {
                    window.electron.ipcRenderer.sendMessage('splash', ['recheck-docker']);
                }}
...
window.electron.ipcRenderer.on('splash', ([eventType, data]) => {
    if (eventType === 'changed') {
        render({
            ...
            dockerStatus: data.dockerStatus ? SplashStatusCheck.OK : SplashStatusCheck.ERROR
        });
    }
